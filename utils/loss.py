...
class ContrasitveLoss(nn.Module):
    ...
    def forward(self, pair_p, pair_s, unpair_p):
        exp_pair = (
            torch.exp(torch.einsum("bnc,bnc->bn", pair_p, pair_s)) / self.temperature
        )
        exp_unpair = (
            torch.exp(torch.einsum("bnc,bnc->bn", unpair_p, pair_s)) / self.temperature
        )
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, target):
        ...
        return sigmoid_focal_loss(
            inputs, target, self.alpha, self.gamma, reduction="mean"
        )
...
class BinaryDiceLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        N_dice_eff = (2 * intersection.sum(1) + self.smooth) / (
            input_flat.sum(1) + targets_flat.sum(1) + self.smooth
        )
